...
from hook.x_hook import XHook
import json
from os.path import join
from pathlib import Path

class XOperator(BaseOperator):
...
if __name__ == "__main__":
    ...
    with DAG(dag_id='x_test', start_date=datetime.now()) as dag:
        ...
        x_operator = XOperator(file_path=join("datalake/x_datascience",
                                              ...
                                              query=query, start_time=start_time, end_time=end_time, task_id='x_operator')
        ti = TaskInstance(task=x_operator)
        x_operator.execute(ti.task_id)